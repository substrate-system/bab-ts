...
// This provides the tools to verify streaming data against a trusted root digest

import { BabDigest } from './digest.js'
import { hashChunk, hashInner, createContexts, createKeyedContexts } from './william3.js'

// Verification metadata for a single chunk
...
export function buildVerificationMetadata(
    ...
    const verificationData: ChunkVerificationData[] = chunks.map((chunk, chunkIndex) => {
        ...
        function traverse(node: TreeNode): boolean {
            ...
            // Check left subtree
            if (node.left && chunkIndex >= node.left.startChunk && chunkIndex < node.left.endChunk) {
            ...
            // Check right subtree
            if (node.right && chunkIndex >= node.right.startChunk && chunkIndex < node.right.endChunk) {
...
export function verifyChunk(
    ...
    for (let i = siblingLabels.length - 1; i >= 0; i--) {
        ...
        const isRoot = (i === 0)
        currentLabel = hashInner(leftLabel, rightLabel, combinedLength, isRoot, innerContext)
    }
